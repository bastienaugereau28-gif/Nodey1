...
const COMPONENT_CONFIGS = {
  ...
  [COMPONENT_TYPES.JOINT]: {
    ...
    render: (color) => (
      <circle cx="0" cy="0" r="0.15" fill={color} stroke="none" />
    ),
  },
};
...
export default function CircuitBuilder() {
  ...
  const nodePoints = useMemo(() => {
    ...
    wires.forEach((wire) => {
      ...
      const startPortDef = startConfig.ports.find(
        (p) => p.id === wire.startPortId
      );
      ...
      const ePos = getRotatedPortPosition(
        endComp.x,
        endComp.y,
        endPortDef,
        endComp.rotation
  ...
  const deleteSelection = () => {
    if (selectedId) {
      if (wires.find((w) => w.id === selectedId)) {
        ...
        setWires(
          wires.filter(
            (w) => w.startCompId !== selectedId && w.endCompId !== selectedId
          )
        );
      }
      setSelectedId(null);
  ...
  const handleGlobalPointerUp = (e) => {
    ...
    if (newCompDrag) {
      ...
      if (
        ...
        const finalPos = getSnappedComponentPosition(
          newCompDrag.type,
          canvasX,
          canvasY,
          0
    ...
    if (isDragging) {
      ...
      if (selectedId && !wires.find((w) => w.id === selectedId)) {
        setElements(
          elements.map((el) => {
            if (el.id === selectedId) {
              const snapped = getSnappedComponentPosition(
                el.type,
                el.x,
                el.y,
                el.rotation
  ...
  const tryConnectWire = (compId, portId) => {
    ...
    if (
      drawingWire.startCompId === compId &&
      drawingWire.startPortId === portId
    )
      return;
    const newWire = {
      id: Date.now().toString(),
  ...
  const handlePortPointerDown = (e, compId, portId) => {
    ...
    if (mode === "wire" || mode === "select") {
      if (drawingWire) {
        ...
        const absPos = getRotatedPortPosition(
          comp.x,
          comp.y,
          portDef,
          comp.rotation
  ...
  return (
    <div
      ref={containerRef}
      className={`relative h-screen font-sans overflow-hidden select-none touch-none ${
        isPhotoMode
          ? "bg-white"
          : isBlueprint
      ...
      {newCompDrag && (
        ...
        >
          <div className="bg-blue-500/20 p-4 rounded-xl border border-blue-400 backdrop-blur-sm shadow-2xl">
            ...
            >
              {COMPONENT_CONFIGS[newCompDrag.type].render(
                isBlueprint ? "white" : "black"
              )}
            </svg>
          </div>
        </div>
      ...
      <div className="w-full h-full relative">
            ...
            >
              ...
              >
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <div
                      className={`mb-2 ${mode === "wire" ? "scale-110" : ""}`}
                    >
                      <Plug size={32} />
                    </div>
                    ...
                    .map((type) => {
                      ...
                      return (
                        <div
                          key={type}
                          onPointerDown={(e) =>
                            handlePalettePointerDown(e, type)
                          }
        ...
        <div className="absolute top-8 left-1/2 transform -translate-x-1/2 z-50 flex items-center gap-4">
          ...
          >
            <button
              ...
              className={`px-6 py-3 rounded-full text-xs font-bold transition-all min-w-[80px] ${
                !isBlueprint
          ...
          >
            <button
              ...
              className={`p-3 rounded-full transition-colors active:bg-black/10 ${
                isBlueprint ? "active:bg-white/10" : ""
              }`}
              title="Zoom Arrière"
            >
              <Minus size={20} />
            ...
            <button
              ...
              className={`px-4 py-2 text-xs font-bold rounded-lg active:bg-black/10 ${
                isBlueprint ? "active:bg-white/10" : ""
              }`}
            ...
            <div
              className={`w-px h-6 mx-2 ${
                isBlueprint ? "bg-slate-600" : "bg-slate-300"
              }`}
            ></div>
        ...
        <div
          className={`w-full h-full ${
            isBlueprint && !isPhotoMode ? "bg-[#1e293b]" : "bg-slate-50"
          }`}
          style={
            isBlueprint && !isPhotoMode
              ? {
        ...
        >
          ...
          >
            ...
            <g transform={`translate(${pan.x}, ${pan.y}) scale(${scale})`}>
              ...
              {/* Wires */}
              {wires.map((wire) => {
                const startComp = elements.find(
                  (e) => e.id === wire.startCompId
                );
                const endComp = elements.find((e) => e.id === wire.endCompId);
                ...
                const endPortDef = endConfig.ports.find(
                  (p) => p.id === wire.endPortId
                );
                ...
                const numArrows = Math.max(
                  1,
                  Math.floor(length / arrowSpacing)
                );
                const arrowPoints = [];
                        ...
                        <g>
                          ...
                          >
                            ...
                            <RotateCw
                              ...
                              y={-8}
                              className={
                                isBlueprint ? "text-slate-300" : "text-blue-600"
                              }
                            />
                          </g>
                          ...
                          >
                            ...
                            <Trash2
                              ...
                              y={-8}
                              className={
                                isBlueprint ? "text-red-400" : "text-red-600"
                              }
                            />
                          </g>
                        </g>
                    ...
                    {(mode === "wire" || isSelected || drawingWire) &&
                      ...
                      config.ports.map((port) => (
                        <circle
                          ...
                          onPointerDown={(e) =>
                            handlePortPointerDown(e, el.id, port.id)
                          }
                          onPointerUp={(e) =>
                            handlePortPointerUp(e, el.id, port.id)
                          }
                        />
                      ))}
                  </g>
